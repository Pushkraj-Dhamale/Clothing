...
exports.getProductsByColor = async (req, res) => {
    try {
        const products = await affiliate.find({ 
            $and: [
...
exports.getProductsByPrice = async (req, res) => {
    try {
        const products = await affiliate.find({ $and: [{ price: { $gte: req.body.lowest } }, { price: { $lte: req.body.uppest } }] });
...
exports.getProductsByStatus = async (req, res) => {
    try {
        const products = await affiliate.find({ status: req.params.status });
...
exports.getProductsBySearch = async (req, res) => {
    try {
        const products = await affiliate.find({ name: { $regex: '.*' + req.params.search + '.*', "$options": "i" } });
...
exports.addProduct = async (req, res) => {
    try {
        const newProduct = await affiliate.create(req.body);
...
exports.updateProduct = async (req, res) => {
    try {
        const product = await affiliate.findByIdAndUpdate(req.params.id, req.body, { new: true });
...
exports.deleteProduct = async (req, res) => {
    try {
        const product = await affiliate.findByIdAndDelete(req.params.id);
